...
├── lua/
│   ├── core/
│   │   ├── basic.lua           # 基础设置（行号、缩进、折叠、透明度、自动刷新等）
...
| | `<leader>ou` | 向上翻半页 | |
| | `<leader>od` | 向下翻半页 | |
| **🍃 Git** | `<leader>gj` | 下一个 hunk | |
| | `<leader>gk` | 上一个 hunk | |
| | `<leader>gs` | 暂存当前 hunk | |
| | `<leader>gr` | 撤销当前 hunk | |
| | `<leader>gP` | 预览当前 hunk 差异 | |
| | `<leader>gb` | 显示行 blame | |
| | `<leader>gd` | diff 当前文件与 index | |
| | `<leader>gS` | 暂存整个 buffer | |
| | `<leader>gR` | 撤销整个 buffer | |
| | `<leader>gt` | 开关行内 blame | |
| | `<leader>gu` | 撤销上一次暂存 | |
...
| 插件 | 用途 |
|------|------|
| catppuccin | Mocha 主题，是否透明由 `basic.lua` 中变量控制 |
| bufferline | 顶部标签式缓冲栏 |
| lualine | 底部状态栏 |
| noice | 浮动命令行 & 通知框 |
...
- 透明度统一由 `lua/core/basic.lua` 中的 `vim.g.transparent_background_enabled` 控制
- `true` 时，以下界面为透明：
  - catppuccin 主题
  - which-key 浮窗
  - Lazy 浮窗
...
- 已启用 `autoread`
- 当文件被外部程序修改时，Neovim 会在以下时机自动执行 `checktime` 并刷新 buffer：
  - `FocusGained`
  - `BufEnter`
  - `CursorHold`
  - `CursorHoldI`
  - `TermClose`
- 文件自动重载后会显示提示：`File changed on disk. Buffer reloaded.`
